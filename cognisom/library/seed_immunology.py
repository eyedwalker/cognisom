...
def _seed_mhc_molecules(store: EntityStore) -> int:
    _mhc_descriptions = {
        "HLA-A*02:01": (
            "Most common MHC class I allele worldwide (~29% Caucasian, ~12% African, ~24% Asian "
            "frequency). HLA-A*02:01 presents 8-10mer peptides with strong preference for leucine/methionine "
            "at position 2 (anchor) and valine/leucine at the C-terminus (anchor position 9). The peptide-"
            "binding groove (alpha-1/alpha-2 domains) accommodates ~10,000 different peptides. Peptides "
            "are generated by the proteasome, transported into the ER by TAP1/TAP2, loaded onto "
            "HLA-A*02:01/beta-2-microglobulin heterodimer by the peptide-loading complex (tapasin, "
            "ERp57, calreticulin), and displayed on the cell surface for CD8+ T cell surveillance. "
            "HLA-A*02:01 is the most extensively studied allele for cancer immunotherapy and neoantigen "
            "vaccine design. Associated with type 1 diabetes and narcolepsy susceptibility."
        ),
        "HLA-B*27:05": (
            "MHC class I allele with the strongest known HLA-disease association: >90% of ankylosing "
            "spondylitis (AS) patients carry HLA-B*27, compared to ~8% of the general population "
            "(relative risk >100). Proposed mechanisms: (1) Arthritogenic peptide hypothesis — HLA-B*27 "
            "presents unique self-peptides that cross-react with microbial antigens (molecular mimicry); "
            "(2) Misfolding hypothesis — B*27 heavy chain forms homodimers that activate KIR3DL2 on "
            "NK/T cells and trigger IL-17-producing immune responses; (3) Unfolded protein response — "
            "B*27 misfolding in the ER activates NF-kappaB and IL-23 production. Also associated with "
            "reactive arthritis (post-Salmonella/Chlamydia/Yersinia), acute anterior uveitis, and "
            "psoriatic arthritis."
        ),
        "HLA-B*57:01": (
            "MHC class I allele associated with elite control of HIV-1 infection (~1/300 HIV+ individuals "
            "maintain undetectable viral load without ART, and ~50% carry B*57:01). B*57:01 presents "
            "immunodominant HIV-1 Gag epitopes (TW10, KF11) that target structurally constrained regions "
            "of the viral capsid, forcing escape mutations with severe fitness costs. Also causes "
            "abacavir hypersensitivity syndrome: abacavir (nucleoside reverse transcriptase inhibitor) "
            "binds non-covalently within the B*57:01 peptide-binding groove, altering the repertoire of "
            "presented self-peptides and triggering a CD8+ T cell-mediated drug reaction. HLA-B*57:01 "
            "screening before abacavir prescription is mandatory (pharmacogenomics standard of care)."
        ),
        "HLA-DRB1*04:01": (
            "MHC class II allele (shared epitope) that is the strongest genetic risk factor for "
            "seropositive rheumatoid arthritis (RA). MHC-II molecules present 13-25mer peptides to "
            "CD4+ T cells. HLA-DR molecules are heterodimers of DRA (invariant) and DRB1 (polymorphic) "
            "chains. DRB1*04:01 contains the 'shared epitope' (amino acids 70-74: QKRAA in DRB1) in "
            "the peptide-binding groove, which preferentially presents citrullinated peptides — "
            "post-translationally modified self-proteins (vimentin, fibrinogen, alpha-enolase) created "
            "by PAD enzymes in inflamed joints. Anti-citrullinated protein antibodies (ACPA) are the "
            "hallmark serological marker of RA. Also associated with type 1 diabetes risk."
        ),
        "HLA-DQ2.5": (
            "MHC class II heterodimer (DQA1*05:01/DQB1*02:01) present in ~90-95% of celiac disease "
            "patients vs. ~25% of the general population. HLA-DQ2.5 has unique peptide-binding "
            "preferences: positions P4, P6, P7 strongly favor negatively charged residues (glutamate). "
            "Wheat gluten peptides (gliadin) are naturally rich in glutamine, which is converted to "
            "glutamate by tissue transglutaminase (tTG2) in the intestinal lamina propria — a "
            "post-translational modification (deamidation) that creates perfect anchor residues for "
            "DQ2.5 binding. Deamidated gliadin peptides presented by DQ2.5 activate gluten-specific "
            "CD4+ T cells, driving villous atrophy and crypt hyperplasia. The only treatment is "
            "lifelong gluten-free diet. DQ2.5 is necessary but not sufficient (present in ~25% of "
            "population, but only ~1% develop celiac disease)."
        ),
        "B2M (beta-2 microglobulin)": (
            "Small (12 kDa) non-polymorphic protein that is an obligate light chain component of all "
            "MHC class I molecules (HLA-A, -B, -C) and non-classical MHC-I (HLA-E, -F, -G, CD1, FcRn, "
            "MR1). B2M is non-covalently associated with the MHC-I heavy chain and is essential for "
            "proper folding, peptide loading, and surface expression. B2M loss (through gene deletion "
            "or mutations) abolishes all MHC-I surface expression — a common immune evasion mechanism "
            "in cancer (~6% of cancers across types). B2M-null tumors cannot be recognized by CD8+ "
            "T cells (no peptide-MHC-I presentation), causing resistance to checkpoint inhibitors and "
            "adoptive T cell therapy. However, B2M-null cells become susceptible to NK cell killing "
            "(missing-self recognition). Serum B2M is elevated in multiple myeloma and is an "
    ...
    for name, mclass, allele, gene, pep_range, dist, presenting, diseases in mhc_molecules:
        entity = MHCMolecule(
            name=name,
            display_name=name,
            description=_mhc_descriptions.get(name, f"MHC class {mclass}. Presents {pep_range[0]}-{pep_range[1]}mer peptides."),
            mhc_class=mclass,
            hla_allele=allele,
            gene_symbol=gene,
