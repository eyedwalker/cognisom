...
resource "aws_cognito_user_pool_client" "web" {
  ...
  # No secret for public web clients (browsers can't securely store secrets)
...
resource "aws_cognito_user_pool_client" "api" {
  ...
  # Must wait for resource server to create scopes first
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.exports.arn}/users/$${cognito-identity.amazonaws.com:sub}/*"
        ]
      }
    ]
  })
